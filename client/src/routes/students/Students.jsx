  ...
  return (
    <>
      ...
      <div className="admin__content-body">
        <div className="students-searchbar">
        ...
        <div className="table-wrapper">
          {
            !isloading && students_data.length > 0 ?
              <table className="students-table">
                ...
                <tbody>
                  {
                    students_data.filter(student => student.name.indexOf(search) !== -1).filter(select => select.status.indexOf(selectedStatus) !== -1).map(student => (
                      <tr key={student._id}>
                        {
                          tableHeaderContent &&
                          tableHeaderContent.map((thitem, index) =>
